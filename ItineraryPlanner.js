...
// Only import map elements if we are on Web (prevents Expo mobile crashes)
...
if (Platform.OS === 'web') {
  ...
  L = require('leaflet');
  // Inject Leaflet CSS via CDN to bypass Expo's CSS bundler bug
...
// Fix for React-Leaflet missing icon bug
...
export default function ItineraryPlanner({ destination }) {
  ...
  const generateItinerary = async () => {
    ...
    try {
      const prompt = `
        ...
        {
          "summary": "A 2-sentence engaging summary.",
    ...
    try {
      const prompt = `
        ...
        Return EXACTLY ONE JSON object matching this structure:
        {
          ...
          "description": "A totally new place they will love.",
          "admission": "Free",
          "cost": 0,
          "ecoTag": "Local Park",
  ...
  // --- 3. EXTRACT COORDINATES FOR THE MAP ---
  // This automatically finds all the pins and calculates the center of the city!
  const mapData = useMemo(() => {
    if (!planData || Platform.OS !== 'web') return { markers: [], center: [0, 0] };
    let markers = [];
    planData.itinerary.forEach(day => {
      day.activities.forEach(act => {
        if (act.lat && act.lng) markers.push(act);
      });
    });
  ...
  if (loading) {
    return (
      <View style={styles.centerContainer}>
        ...
        <Text style={styles.loadingText}>Synthesizing spatial data & budgeting...</Text>
      </View>
    );
  }
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {Platform.OS === 'web' && mapData.markers.length > 0 && (
          <View style={styles.mapContainer}>
            {/* The key={Date.now()} forces the map to refresh if the center changes drastically */}
              ...
              {mapData.markers.map((marker, index) => (
                <Marker key={marker.id || index} position={[marker.lat, marker.lng]} icon={customIcon}>
                  <Popup>
                    ...
                    {marker.time} - {marker.admission}<br/>
                    <i>{marker.ecoTag}</i>
                  </Popup>
                </Marker>
              ))}
              ...
              <View key={act.id} style={styles.activityRow}>
                ...
                <View style={styles.detailsCol}>
                  ...
                  <TouchableOpacity style={styles.replaceBtn} onPress={() => replaceActivity(dayIdx, actIdx, act)} disabled={replacingId === act.id}>
                    {replacingId === act.id ? <ActivityIndicator size="small" color="#D97706" /> : <Text style={styles.replaceBtnText}>🔄 Dislike this? Replace Activity</Text>}
                  </TouchableOpacity>
...
const styles = StyleSheet.create({
  container: { flexGrow: 1, padding: 20, backgroundColor: '#F5F7FA', alignItems: 'center' },
  ...
  // NEW MAP STYLE
  mapContainer: { width: '100%', maxWidth: 800, height: 350, backgroundColor: '#E0E0E0', borderRadius: 12, overflow: 'hidden', marginBottom: 20, borderWidth: 1, borderColor: '#DDD', zIndex: 1 },
  ...
  budgetBox: { backgroundColor: '#E8F5E9', padding: 20, borderRadius: 12, width: '100%', maxWidth: 800, marginBottom: 20, borderWidth: 1, borderColor: '#A5D6A7' },
  ...
  card: { backgroundColor: '#FFF', padding: 20, borderRadius: 12, width: '100%', maxWidth: 800, marginBottom: 15, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 5, elevation: 2 },
  ...
  replaceBtn: { backgroundColor: '#FEF3C7', paddingVertical: 8, paddingHorizontal: 12, borderRadius: 6, alignSelf: 'flex-start', borderWidth: 1, borderColor: '#FDE68A' },
